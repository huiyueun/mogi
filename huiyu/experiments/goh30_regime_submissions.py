...
OUT_DIR      = os.environ.get('OUT_DIR', './outputs/goh30_regime_submissions')
...
# ── 30모델 예측 수집(GRU10 + ODE10 + H10) ──
preds = []
for k in range(N_EACH): preds.append(predict_resid(f'{MODELS_DIR}/phaseG_full_{k}.pt', AttnGRU))
for k in range(N_EACH): preds.append(predict_resid(f'{MODELS_DIR}/phaseODE_full_{k}.pt', ODEModel))
for k in range(N_EACH): preds.append(predict_h(f'{MODELS_DIR}/phaseH_full_{k}.pt'))
print(f'블렌드 {len(preds)}모델 (GRU{N_EACH}+ODE{N_EACH}+H{N_EACH})')

ens = np.mean(preds, 0)   # 30모델 등가중 평균
kalman = kalman_cv_predict(X_test_raw)
masks = regime_masks(X_test_raw)

print("regime counts:", {k: int(v.sum()) for k, v in masks.items()})
write_submission("case00_goh30_original", ens, ids)
write_submission("case01_high_speed_kalman_005", masked_blend(ens, kalman, masks["high_speed"], 0.05), ids)
write_submission("case02_high_speed_kalman_010", masked_blend(ens, kalman, masks["high_speed"], 0.10), ids)
write_submission("case03_high_speed_kalman_015", masked_blend(ens, kalman, masks["high_speed"], 0.15), ids)
write_submission("case04_high_speed_kalman_020", masked_blend(ens, kalman, masks["high_speed"], 0.20), ids)
write_submission("case05_high_speed_kalman_100", masked_blend(ens, kalman, masks["high_speed"], 1.00), ids)
write_submission("case06_high_noise_kalman_005", masked_blend(ens, kalman, masks["high_noise"], 0.05), ids)
write_submission(
    "case07_high_speed_or_noise_kalman_010",
    masked_blend(ens, kalman, masks["high_speed"] | masks["high_noise"], 0.10),
    ids,
)
case08 = ens.copy()
case08[masks["high_noise"]] = 0.95 * ens[masks["high_noise"]] + 0.05 * kalman[masks["high_noise"]]
case08[masks["high_speed"]] = 0.90 * ens[masks["high_speed"]] + 0.10 * kalman[masks["high_speed"]]
write_submission("case08_high_speed_010_high_noise_005", case08, ids)
